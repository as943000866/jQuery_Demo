 ...
 */
    ...
    function nextPage(flag) {
        ...
        if(typeof flag === 'boolean'){
            realOffset = flag?-offset:offset
        }else{
            realOffset = - (flag - index) * 600
        }




        var targetOffect = currentOffect + realOffset

        //循环移动分段偏移量, 直到到达指定位置
        var intervalId = setInterval(function () {

            //目标偏移量
            var itemOffect = realOffset/(time/itemTime)
            currentOffect += itemOffect
